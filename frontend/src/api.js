...
const demoStatistics = () => {
  ...
  return { cities: demoCities.length, reported, investigation, trial, convicted, acquitted,
    case_status: [{ status:"ACQUITTED",count:acquitted },{ status:"CONVICTED",count:convicted },{ status:"INVESTIGATION",count:investigation },{ status:"TRIAL",count:trial }],
...
export const api = {
  ...
  cities: async () => { try { const data=normalizeCities(await request("/cities/")); return data.length?data:demoCities; } catch { return demoCities; } },
  areas: async (city="") => { try { const data=await request(`/areas/${city?`?city=${encodeURIComponent(city)}`:""}`); if(Array.isArray(data)&&data.length)return data; } catch {} return city?demoAreas.filter(a=>a.city.toLowerCase()===city.toLowerCase()):demoAreas; },
  cases: async ({search="",status="",city=""}={}) => { try { const p=new URLSearchParams(); if(search)p.set("search",search); if(status)p.set("status",status); if(city)p.set("city",city); const q=p.toString(); return await request(`/cases/${q?`?${q}`:""}`); } catch { return demoCases.filter(c=>(!search||`${c.id} ${c.city} ${c.area}`.toLowerCase().includes(search.toLowerCase()))&&(!city||c.city.toLowerCase()===city.toLowerCase())&&(!status||c.status.toUpperCase().replaceAll(" ","_")===status)); } },
  statistics: async () => { try{return await request("/statistics/");}catch{return demoStatistics();} },
